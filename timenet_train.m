function paramsp = timenet_train( runName, runDesc, paramsp, Win, bin, resumeFile, maxepoch, indata, outdata, numchunks, layersizes, layertypes, mattype, rms, errtype, hybridmode, weightcost, decay, jacket , timeLabels)

%timenet mod to include timeLabels vector for each frame
indata = cat(1,indata,timeLabels);
outdata = cat(1,outdata,timeLabels);
%Test set has been removed from code

% paramsp = nnet_train_2( runName, runDesc, paramsp, Win, bin, resumeFile, maxepoch, indata, outdata, numchunks, layersizes, layertypes, mattype, rms, errtype, hybridmode, weightcost, decay, jacket)
%
...
if ~isempty( resumeFile )
    ...
    llrecord = zeros(maxepoch,1);
    errrecord = zeros(maxepoch,1);
...
if isempty(paramsp)
    if ~isempty(Win)
        ...
        for i = 1:numlayers
            ...
            %incoming
            for j = 1:(layersizes(i+1)-1)  %Do not assign random inputs to last unit in next layer
                idx = ceil((layersizes(i)-1)*rand(1,numconn)); %Do not assign random outputs from last unit in this layer
                Wtmp{i}(j,idx) = randn(numconn,1)*initcoeff;
            end
            
            Wtmp{i}(end,end) = 1; %initialize timelabel weights to 1
            
        end
        
        for i=1:numlayers %Correct timelabel weights
           
            Wtmp{1}(end,end)=1e-1;
            if strcmp( layertypes{i}, 'linear' )
                Wtmp{i}(end,end) = 1e1;
                if i<numlayers
                Wtmp{i+1}(end,end)=1e-1;
                end
            end
